...
ctx.symbolizeMemory(MemoryAccess(0x7fffffffe146, 8))
addr = 0x55555555514d
while addr in code:
	inst = Instruction()
	...
	if inst.isSymbolized():
		...
		#print("  reg read:" + str(inst.getReadRegisters()))
		#print("  reg write:" + str(inst.getWrittenRegisters()))
		#print("  mem read:" + str(inst.getLoadAccess()))
		#print("  mem write:" + str(inst.getStoreAccess()))
		#for expression in inst.getSymbolicExpressions():
		#	print("\t" + str(expression.getAst()))
		if inst.isBranch():
			#if ctx.getPathConstraints()[0].isMultipleBranches():
			...
			#ctx.getPathConstraints()[0].getBranchConstraints()
	else:
		print('[*] %s' % str(inst))
