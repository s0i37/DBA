...
void print_solve(triton::uint512 value, unsigned int size)
{
	for(unsigned int i = 0; i < size/8; i++)
		printf("%02X", *((unsigned char *)(value.backend().limbs())+i));
	printf("\n");
}
int main(void)
{
	...
	api.symbolizeMemory(MemoryAccess(0x7fffffffe146, 8));
	...
	while( (op = in_area(addr)) != -1 )
	{
		...
		{
			std::cout << "[symbolic] 0x" << std::hex << addr << ": " << inst.getDisassembly() << std::endl;
			if(inst.isBranch())
			{
				auto model = api.getModel(api.getPathPredicate());
				for (auto it = model.begin(); it != model.end(); it++)
				{
				    //std::cout << "  - Variable name: " << it->second.getVariable()->getName() << std::endl;
				    print_solve(it->second.getValue(), it->second.getVariable()->getSize());
				}
			}
		}
		else
